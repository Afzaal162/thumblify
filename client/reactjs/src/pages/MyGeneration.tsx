...
import { ArrowUpRight, ArrowUpRightIcon, DownloadIcon, TrashIcon } from "lucide-react";
...
const MyGeneration = () => {
  ...
  const fetchThumbnails = async () => {
    try {
      setLoading(true)
const { data } = await api.get('/api/thumbnail/user'); 
  ...
  const handleDownload = (image_url: string) => {
    ...
    link.download = 'thumbnail.jpg'; // optional but recommended
...
{/* Show empty state only if not loading and thumbnails are empty */}
{!loading && thumbnails.length === 0 && (
  <div className="text-center py-24">
    ...
    <p className="text-sm text-zinc-400 mt-2">
      Generate Your First Thumbnail and See Here
    </p>
  </div>
)}
            ...
            {thumbnails.map((thumb: IThumbnail) => {
              ...
              return (
                <div
                  ...
                  bg-white/6 border-white/10 transition shadow-xl break-inside-avoid"
                >
                  {/* Image */}
                  <div
                    className={`relative overflow-hidden rounded-t-2xl ${aspectClass} bg-black`}
                  >
                    {/* ✅ fixed ternary syntax */}
                    {thumb.image_url ? (
                      <img
                        src={thumb.image_url}
                        alt={thumb.title}
                    ...
                    {thumb.isGenerating && (
                      <div
                        ...
                        items-center justify-center text-sm font-medium text-white"
                      >
                        Generting....
                      </div>
                    )}
                  ...
                  <div onClick={(e) => e.stopPropagation()}
                    className="absolute bottom-2 right-2 hidden group-hover:flex sm:group-hover:flex max-sm:flex gap-1.5">
                    <TrashIcon onClick={() => deleteDownload(thumb._id)} className="size-6 bg-black/50 p-1 rounded hover:bg-pink-600 transition-all" />
                    ...
                    hover:bg-pink-600 transition-all" /></Link>
